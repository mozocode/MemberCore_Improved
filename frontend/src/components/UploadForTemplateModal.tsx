...
async function fileToDataUrl(file: File): Promise<string> {
  if (file.type.startsWith('image/')) {
    return compressImageFile(file, { maxSize: 1200, maxBytes: 900_000 })
  }
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl bg-zinc-900 border border-zinc-700">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            ...
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={file?.name || 'Document title'} className="mt-1 bg-zinc-800 border-zinc-700" />
          </div>
          <div>
