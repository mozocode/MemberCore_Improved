...
const DIRECTORY_FILTERS_KEY = 'orgDirectoryFilters'
...
function getInitialFilters(orgId: string | undefined, fromEventDetail: boolean): DirectoryFilters {
  if (!fromEventDetail || !orgId) return emptyFilters
  try {
    const stored = sessionStorage.getItem(`${DIRECTORY_FILTERS_KEY}_${orgId}`)
    if (stored) {
      return { ...emptyFilters, ...(JSON.parse(stored) as DirectoryFilters) }
    }
  } catch {
    /* ignore */
  }
...
export function OrgDirectory() {
  ...
  const fromEventDetail = (location.state as { fromEventDetail?: boolean } | null)?.fromEventDetail
  const [userOrg, setUserOrg] = useState<Org | null>(null)
  const [events, setEvents] = useState<DirectoryEvent[]>([])
  const [loading, setLoading] = useState(true)
  const [filters, setFilters] = useState<DirectoryFilters>(() =>
    getInitialFilters(orgId, !!fromEventDetail),
  )
  ...
  useEffect(() => {
    if (userOrg && !autoFilterApplied && !fromEventDetail) {
  ...
  }, [userOrg, autoFilterApplied, fromEventDetail])

  // Persist filters when leaving Directory; clear when coming from another org tab (not from event)
  useEffect(() => {
    if (!orgId) return
    if (!fromEventDetail) sessionStorage.removeItem(`${DIRECTORY_FILTERS_KEY}_${orgId}`)
    return () => {
      sessionStorage.setItem(`${DIRECTORY_FILTERS_KEY}_${orgId}`, JSON.stringify(filters))
    }
  }, [orgId, filters, fromEventDetail])

  const filteredEvents = applyClientSideFilters(events, filters)
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {/* Community filter banner removed — info already shown above */}
        ...
        <div className="space-y-4">
          {filteredEvents.map((event) => (
            <button
              ...
              className="w-full text-left rounded-xl bg-zinc-900 border border-zinc-700 p-4 hover:border-zinc-600 transition-colors flex gap-4 items-center cursor-pointer group"
            >
              {event.cover_image && (
                <img
              ...
              <ChevronRight size={20} className="text-zinc-500 shrink-0 group-hover:text-zinc-400" />
            </button>
          ))}
        </div>
      )}
