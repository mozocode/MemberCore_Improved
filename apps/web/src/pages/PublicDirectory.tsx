...
export function PublicDirectory() {
  ...
  useEffect(() => {
    ...
    navigate(`/events/${eventId}${payment}`, { replace: true, state: { fromPublicDirectory: true } })
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={cn(
              ...
              filters.orgType || filters.culturalIdentity || filters.sportType || filters.dateRange || filters.radius || filters.state || (filters.city?.trim() ?? '')
            ...
            <div className="max-w-2xl mx-auto space-y-4">
              {filteredEvents.map((event) => (
                <button
                  key={event.id}
                  type="button"
                  onClick={() => navigate(`/events/${event.id}`, { state: { fromPublicDirectory: true } })}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-4 mt-2 text-xs text-zinc-500">
                      {event.is_paid ? (
                        <span>${Number(event.price ?? 0).toFixed(2)} per ticket</span>
                      ) : (
                        <>
